...
<body class="relative min-h-screen w-full overflow-hidden">
    ...
    <div class="absolute h-full left-[78px] transition-all duration-500 ease content" style="width: calc(100% - 78px);">
        <div class="mr-5 ml-[130px] my-5">
            ...
            <!-- POP UP ADD SHIFT START -->
            <div id="shiftPopup" class="fixed inset-0 bg-graysimple bg-opacity-50 flex items-center justify-center z-50">
                <div class="bg-white p-6 rounded-lg w-[400px] shadow-lg">
                    <h2 class="text-2xl font-semibold mb-4">Add Shift</h2>
                    <form id="addShiftForm" method="post">
                       <div class="mb-4">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="time" id="start_time" name="start_time" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                        </div>
                        <div class="mb-4">
                            <label for="end_time" class="block text-sm font-medium text-gray-700">End Time</label>
                            <input type="time" id="end_time" name="end_time" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                        </div>
                        <div class="flex justify-end mt-6">
                            <button type="button" id="closeShiftPopupBtn" onclick="closeShiftPopup()" class="mr-2 px-4 py-2 bg-darksystem text-white hover:bg-gray-600 rounded-lg">Cancel</button>
                            <button type="submit" name="submit" class="px-4 py-2 bg-darksystem text-white hover:bg-gray-600 rounded-lg">Add</button>
                        </div>
                    </form>
                </div>
            </div>
            <!-- POP UP ADD SHIFT END -->
